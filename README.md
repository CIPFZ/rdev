...
安全演进 Batch A 已完成 Phase 1：项目配置摘要审批、集中 SSH destination/`RemoteDir` 校验、bootstrap 位置参数化、配置 no-follow 原子写，以及 rsync `--`/路径验证。Phase 0 只落了本批需要的 `SECURITY.md`、安全事件/低基数指标 seam、characterization/race 测试和支持矩阵基础；连接 simulator、完整 doctor/trace、storage fixture、真实 sshd harness 与 fuzz 基础设施仍按[演进规划](docs/rdev-evolution-security-plan.md)后续实施。
