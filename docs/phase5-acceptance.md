...
| P5-08 | Reserved control handlers/queues, dedicated on-demand bulk transport, actual payload byte pacing, and real baseline-versus-bulk control p95 assertions in `make remote-qos` | In progress | Initial real 20-process SLO run passed (ratios 1.51 and 1.39); committed repeated-run evidence is recorded below; wider payload/sync/streaming and independent review remain |
