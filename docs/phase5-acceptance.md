...
| P5-03 | `rdevd` owns `client.Client`, host registry, secrets, configurable agent lookup, wire dispatch; real CLI `ping`, `exec`, `read`, `write`, `capability`, `job list`, and `job status` process tests through Unix broker with allow/deny and dispatcher evidence; pipelined request test; `make stress-broker` runs 20-client wire test 100 times | In progress | Remaining CLI/MCP commands and multi-process remote-session benchmark remain |
