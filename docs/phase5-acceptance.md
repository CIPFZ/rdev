...
|---|---|---|
| C1: Finish shared routes | Shared push/pull/delete execute only a retained, approved source/destination plan; exact owner/host/policy binding, substitution negatives, cancellation, crash/restart without duplicate mutation, durable outcomes and private audit correlation; remaining shared session administration preserves other owners | P5-03/04/05/12/13/14/16; authority, approval and mutation gates |
...
Earlier dated follow-up paragraphs preserve historical evidence and limitations.
This closeout scope and the current primary table govern remaining Phase5 work;
a historical reference to a broader matrix is not an additional completion gate.
...
| P5-03 | `rdevd` owns `client.Client`, host registry, secrets, configurable agent lookup, wire dispatch; real CLI job/file/capability process tests through Unix broker; broker-backed `rdev serve` MCP ping/exec/read/write/capability and full job start/list/status/logs/stop/wait/rm tools with registration test; pipelined request test; `make stress-broker` passed 20-client wire test 100 times, with an additional 1000-run stress pass; Linux amd64 binary remote smoke passed; `make remote-session-benchmark` ran 3 x 500 real SSH requests from 20 independent client processes, with one SSH child, one remote PID and 20 distinct remote principal IDs | In progress | Shared session benchmark and owner-scoped secret store are proven; C1 still needs approved shared sync execution and remaining shared session administration; C5 independent review remains. |
| P5-04 | Owner validation, handshake-declared and connection-level owner binding, wire client/project binding, persisted job owner, required daemon authentication by default, 0600 key provisioning, expiring HMAC token bound to owner identity, live SIGHUP revocation, and real daemon credential lifecycle tests | Runtime passed; review pending | Real request identity, principal lifecycle, exact-project secret/job scope and file import pass; C1 must retain these boundaries for remaining routes; C5 remains. Declarative delegation is follow-up scope. |
| P5-05 | Per-connection context cancellation, broker frontend `DoContext` cancellation that closes only the local socket, shared client pool, disconnect integration test, transport cancellation/late-frame tests, full race test, and `make remote-lifecycle` real daemon/OpenSSH retry cancellation | Runtime passed; review pending | Real retry, frontend/SIGKILL, shared-wait and rsync-preview cancellation preserve other owners; C1/C2 must cover completed shared sync and mixed workloads; C5 remains. |
| P5-06 | Atomic fair admission with separate global/host/owner execution and queue limits, reserved control capacity, cancellable queued work and owner-only status snapshots; real 20-process remote bulk overload and owner SIGKILL tests | In progress | File-I/O quotas, ingress limits, wait reservations, warm-host bounds and preview worker budgets have runtime evidence; C2 must cover combined exec/job/sync saturation and C3 remaining sync accounting; C5 remains. |
| P5-07 | Eligible-owner weighted scheduling, removal of empty/canceled queues, live weight changes for existing backlog; real continuously queued SSH file reads with 3:1 weights reversed to 1:3 by SIGHUP | In progress | Sustained real file-I/O weighted fairness passes; C2 must demonstrate progress under long exec/job wait/status/sync together; C5 remains. |
| P5-08 | Reserved control handlers/queues, dedicated on-demand bulk transport, actual payload byte pacing, and real baseline-versus-bulk control p95 assertions in `make remote-qos` | In progress | Repeated real file-bulk control SLO passes, including 512 retained secret versions; C2/C3 must include actual sync payload pacing, counters and the unchanged 2-times control p95 limit; C5 remains. |
| P5-09 | Bounded owner-scoped wait coalescing with independently canceled subscribers and observation leases; real 20-process wait test covers initiator/follower SIGKILL, reconnect, SIGHUP, shared terminal operation ID/tail and prompt SIGTERM observation cancellation; private bounded event history persists state changes in the observation worker, including zero-subscriber completion, and exposes job.events plus CLI/MCP cursor replay | Runtime passed; review pending | Real 20-process coalescing, reconnect, zero-subscriber completion and durable CLI/MCP cursor replay pass; C5 remains. Full push streaming and extended retention certification are follow-up scope. |
| P5-10 | Versioned private bounded job registry with persist-before-publish updates and fail-closed uncertain commits; real two-project detached jobs survive repeated daemon SIGKILL, SSH-unavailable recovery, SIGHUP and remote-delete/local-rename failure (`make remote-jobs`); `make remote-mutation` exercises execution-before-ACK SIGKILL, SSH outage, durable job rediscovery, append replay refusal, intent rename failure, CLI/MCP stable IDs and exact-project outcome isolation | Runtime passed; review pending | Acknowledged/pre-ACK jobs, outages, removal failures and 12 real old/new daemon-and-agent upgrades pass; C5 remains. Full automated schema/rollback compatibility belongs to P8-06. |
| P5-11 | Atomic lease admission/pool detachment, cleanup outside the lease lock, in-flight accounting, runtime `IdleTTL` reload, and six real remote lifecycle cycles over 65 seconds (normal exit/SIGKILL, 60 new-owner requests) | Runtime passed; review pending | Six real normal/SIGKILL lease cycles, zero-subscriber observation lease and bounded observation shutdown pass; C1/C2 must retain these guarantees with completed shared sync; C5 remains. |
| P5-12 | Default-deny owner policy, persisted grants, capability-scoped decisions, policy administration RPC, connection owner switching rejection, and runtime HMAC principal-token validation, private strict policy loading, durable grant/revoke, exact-host grants, server-derived capability, policy digests and `make remote-policy` real queued/crash/denial/failure-injection evidence | Runtime passed; review pending | Real default deny, pre-queue stable policy snapshot, exact host/project/capability and host-scoped administration pass, including secret use and reserved Fleet denial; C1 adds remaining routes under the same checks; C5 remains. |
| P5-13 | Server-mandatory approval for every mutating wire operation, administrator issuance RPC/CLI, exact owner/host-session/request/policy binding, single use/expiry, pre-dial target validation, and real SSH substitution/replay/restart/privacy tests | In progress | Exact wire/secret approval, substitution, expiry, replay and restart negatives pass; C1 must bind shared sync mutations to retained source and destination plans; C5 remains. |
| P5-14 | Exact owner audit fingerprints and wire approval/policy correlation; bounded async sink, private bounded recovery, torn-tail repair, visible rotation/write/sync errors and drops, owner-query durability barrier and separately authorized sink health | In progress | Private owner queries, per-request decision/approval/outcome correlation, 600-second rotation/recovery soak and explicit crash-tail uncertainty pass; C1 must correlate the remaining shared routes; C5 remains. Wider retention/power-loss certification is follow-up scope. |
| P5-15 | Readiness gate, optional readiness file, flock recovery, signal-driven listener close, manager syntax checks (`systemd-analyze verify`/`plutil -lint` when installed), reproducible `make smoke-rdevd`, and fail-fast `make remote-smoke`; `make remote-phase5-runtime` runs actual daemon lifecycle/credential tests and systemd user service installation on `service-deploy` | In progress | Actual Linux systemd lifecycle passes; C4 needs real macOS launchd installation/recovery and peer-identity coverage; C5 remains. |
| P5-16 | Private bounded JSON parse/validate, rejected invalid/null/unknown config, live key rotation only after valid config, preservation of malformed state on startup failure, lock retained through shutdown; real daemon startup/reload/crash tests | In progress | Invalid reload preserves active config; durable policy/job/append recovery and bounded held-response shutdown pass; C1 must carry the same guarantees through shared sync mutation; C5 remains. |


The final Phase5 gate requires runtime evidence and passing targeted tests for
every item, completion of C1–C6 and the nine Multi Agent conditions, including
independent review. A runtime-pass label alone does not satisfy the final gate.
...
| Destructive approval binds exact target snapshot and digest | Real SSH mandatory write approval, owner/host/parameter substitution, expiry/replay/policy-change/restart negatives and audit correlation | Wire and secret runtime passed; C1 approved shared sync execution and C5 remain |
...
| P5-03 | `b5e9422`, `08ff4fb`, `a47c59a`, `74da502`, `c5707ee`, `21996d4`, `6d2eeb3`, `5e2d9dc` | Three 20-process / 500-call SSH shared-session benchmarks; bounded 100-alias host pool and fail-closed broker routes |
...
| P5-10 | `e3ac9c2`, `bd03436`, `a96b359`, `a3ccb8d`, `c5707ee`, `21996d4`, `4ded2a4` | Acknowledged and pre-ACK job/append recovery, fresh-agent durable tombstones, CLI/MCP queries, owner isolation and twelve real daemon/agent upgrades |
