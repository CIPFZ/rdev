...
| P5-03 | `rdevd` owns `client.Client`, host registry, secrets, configurable agent lookup, wire dispatch; pipelined request test; `make stress-broker` runs 20-client wire test 20 times | In progress | Installed frontend adoption and multi-process remote-session benchmark remain |
