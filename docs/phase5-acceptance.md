...
| P5-03 | `rdevd` owns `client.Client`, host registry, secrets, configurable agent lookup, wire dispatch; `make stress-broker` runs 20-client wire test 20 times | In progress | Installed frontend adoption and multi-process remote-session benchmark remain |
...
| P5-06 | Quota admission held through dispatch, per-host accounting, bounded queued admission, and 20-client stress target | In progress | Longer duration pressure beyond 20 repeated runs is still required |
