...
| P5-03 | `rdevd` owns `client.Client`, host registry, secrets, configurable agent lookup, wire dispatch; real CLI job/file/capability process tests through Unix broker; broker-backed `rdev serve` MCP ping/exec/read/write/capability/job-start/job-list/job-status tools with registration test; pipelined request test; `make stress-broker` runs 20-client wire test 100 times | In progress | Remaining MCP job tools and multi-process remote-session benchmark remain |
| P5-04 | Owner validation, handshake-declared and connection-level owner binding, wire client/project binding; persisted job owner | In progress | Same-UID raw-client initial principal spoofing still needs a signed principal/token mechanism |
...
| P5-15 | Readiness gate, optional readiness file, flock recovery, signal-driven listener close, systemd/launchd templates validated by test, and reproducible `make smoke-rdevd` | In progress | Manager-specific systemd/launchd installation tests are still required |
