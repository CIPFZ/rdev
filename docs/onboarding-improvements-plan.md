...
| R6 / M2 P1 | Use configured target user (including root); no implicit sudo, password or sshd changes. | Existing host is configured as `service-deploy`; no new privilege escalation path added | Read-only `rdev exec service-deploy -- id -u` returned 0; `.ssh`/`authorized_keys` modes inspected as 700/600; no files changed | 2026-09-13 evidence / partial; bootstrap implementation pending |
